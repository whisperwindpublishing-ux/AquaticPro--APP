...
function lm_register_bulk_email_route() {
    // Bulk email endpoint (existing)
    register_rest_route( 'lm/v1', '/send-unsent-evaluations', [
        'methods'             => 'POST',
        'callback'            => 'lm_handle_bulk_email_request',
        'permission_callback' => 'lm_email_permission_callback',
    ] );
    
    // Single evaluation email endpoint (new)
    register_rest_route( 'lm/v1', '/evaluations/(?P<id>\d+)/send-email', [
        'methods'             => 'POST',
        'callback'            => 'lm_handle_single_email_request',
        'permission_callback' => 'lm_email_permission_callback',
        'args'                => [
            'id' => [
                'required'          => true,
    ...
    // Bulk update emailed flag (mark sent/unsent)
    register_rest_route( 'lm/v1', '/evaluations/bulk-update-emailed', [
        'methods'             => 'POST',
        'callback'            => 'lm_handle_bulk_update_emailed',
        'permission_callback' => 'lm_email_permission_callback',
        'args'                => [
            'ids' => [
                'required'          => true,
...
/**
 * Permission callback for email endpoints.
 */
function lm_email_permission_callback() {
    // Priority 1: Super admins always have access.
    if ( current_user_can( 'manage_options' ) ) {
        return true;
    }

    // Priority 2: Check job role lesson management permissions
    // Users need 'moderate_all' permission to send evaluation emails
    if ( function_exists( 'lm_user_can' ) ) {
        return lm_user_can( 'moderate_all' );
    }
    
    // Fallback: Check against the custom roles saved in the settings (legacy support).
...
/**
 * Handles single evaluation email request.
 *
 * @param WP_REST_Request $request The REST request object.
 * @return WP_REST_Response|WP_Error Response indicating success or failure.
 */
function lm_handle_single_email_request( $request ) {
    $evaluation_id = $request->get_param( 'id' );
    
    // Verify evaluation exists
    $evaluation = get_post( $evaluation_id );
    if ( ! $evaluation || $evaluation->post_type !== 'lm-evaluation' ) {
        return new WP_Error(
            'invalid_evaluation',
            __( 'Evaluation not found.', 'lesson-management' ),
            [ 'status' => 404 ]
        );
    }
    
    // Send the email
    $result = lm_send_single_evaluation_email( $evaluation_id );
    ...
    // Mark as sent
    update_post_meta( $evaluation_id, 'emailed', true );
    
    return new WP_REST_Response( [
        'success' => true,
        'message' => sprintf(
            __( 'Email sent successfully to %s', 'lesson-management' ),
            $result['email']
        ),
        'email'   => $result['email'],
    ], 200 );
...
function lm_send_single_evaluation_email( $evaluation_id ) {
    ...
    // Build the public swimmer progress link using path-based URL (avoids email encoding issues)
...
/**
 * Handles bulk update of the emailed flag on evaluations.
 * Allows marking evaluations as sent or unsent (to put them back in the send queue).
 *
 * @param WP_REST_Request $request
 * @return WP_REST_Response
 */
function lm_handle_bulk_update_emailed( $request ) {
    $ids     = $request->get_param( 'ids' );
    $emailed = (bool) $request->get_param( 'emailed' );

    if ( empty( $ids ) ) {
        return new WP_REST_Response( [ 'message' => 'No evaluation IDs provided.' ], 400 );
    }

    $updated = 0;
    $errors  = [];

    foreach ( $ids as $evaluation_id ) {
        $post = get_post( $evaluation_id );
        if ( ! $post || $post->post_type !== 'lm-evaluation' ) {
            $errors[] = "Evaluation #{$evaluation_id} not found.";
            continue;
        }
        ...
        if ( $emailed ) {
            ...
            delete_post_meta( $evaluation_id, 'emailed' );
        }
        $updated++;
    }

    $action = $emailed ? 'marked as sent' : 'marked as unsent';

    return new WP_REST_Response( [
        'success' => true,
        'message' => sprintf( '%d evaluation(s) %s.', $updated, $action ),
        'updated' => $updated,
        'errors'  => $errors,
    ], 200 );
}