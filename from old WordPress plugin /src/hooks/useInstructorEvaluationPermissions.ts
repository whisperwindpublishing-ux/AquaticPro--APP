...
export interface EffectiveInstructorEvaluationPermissions {
    canView: boolean;
    canCreate: boolean;
    canEdit: boolean;
    canDelete: boolean;
...
/**
 * Hook to get the current user's effective instructor evaluation permissions
 * based on their assigned job roles. Uses OR logic - if ANY assigned role
 * grants a permission, the user has that permission.
 */
export function useInstructorEvaluationPermissions(currentUserId?: number): EffectiveInstructorEvaluationPermissions {
    const [permissions, setPermissions] = useState<EffectiveInstructorEvaluationPermissions>({
        canView: false,
        canCreate: false,
        canEdit: false,
        canDelete: false,
        ...
        const fetchPermissions = async () => {
            ...
            try {
                ...
                const aggregatedPermissions = userRoles.reduce((acc: PermissionState, role: any) => {
                    if (role.instructorEvaluationPermissions) {
                        return {
                            canView: acc.canView || role.instructorEvaluationPermissions.canView,
                            canCreate: acc.canCreate || role.instructorEvaluationPermissions.canCreate,
                            canEdit: acc.canEdit || role.instructorEvaluationPermissions.canEdit,
                            canDelete: acc.canDelete || role.instructorEvaluationPermissions.canDelete,
                            canModerateAll: acc.canModerateAll || role.instructorEvaluationPermissions.canModerateAll
                        };
                    }
                    return acc;
                ...
                console.error('Failed to fetch instructor evaluation permissions:', err);
                setPermissions({
                    canView: false,
                    canCreate: false,
                    canEdit: false,
