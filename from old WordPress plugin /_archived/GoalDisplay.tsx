...
}> = ({ tasks, initiatives, onUpdateTask, onDeleteTask, onAddTask, activeId, isReadOnly, goalTitle, participants = [], focusedTaskId }) => {
        ...
        <div className="ap-space-y-4">
            ...
            {!isReadOnly && (
                <div className="ap-flex ap-justify-end">
                    <Button onClick={onAddTask} variant="link" size="sm" leftIcon={<PlusCircleIcon className="ap-h-5 ap-w-5" />}>
                        Add Task
                    </Button>
                </div>
            )}
            {sortedInitiativeIds.map(initiativeId => {
                ...
                return (
                    <DroppableInitiativeContainer key={initiativeId} id={initiativeId} title={initiative ? initiative.title : 'No Initiative'}>
...
const MeetingList: React.FC<{
...
}> = ({ meetings, initiatives, tasks, onUpdateMeeting, onDeleteMeeting, onAddMeeting, isReadOnly, currentUser, participants, focusedMeetingId }) => {
    ...
    return (
        <div className="ap-space-y-4">
            ...
            {!isReadOnly && (
                <div className="ap-flex ap-justify-end">
                    <Button onClick={onAddMeeting} variant="link" size="sm" leftIcon={<PlusCircleIcon className="ap-h-5 ap-w-5" />}>
                        Add Meeting
                    </Button>
                </div>
            )}
                        ...
                        {!isReadOnly && currentUser && (
                            <div className="ap-bg-white ap-rounded-lg ap-shadow ap-p-4">
                                ...
                                <div className="ap-flex ap-flex-wrap ap-justify-between ap-items-center ap-gap-2 ap-pt-2 ap-border-t ap-border-gray-200 ap-mt-2">
                                    ...
                                    <input type="file" ref={fileInputRef} onChange={handleFileUpload} className="ap-hidden" />
...
const DroppableInitiativeContainer: React.FC<{ id: string; title: string; children: React.ReactNode }> = ({ id, title, children }) => {
    ...
    return (
        <div ref={setNodeRef} style={style} className="ap-bg-gray-50/50 ap-rounded-lg ap-p-3 ap-transition-colors">
            <h3 className="ap-text-lg ap-font-semibold ap-mb-2 ap-break-words">{title}</h3>
            {children}
        </div>
    );
};
